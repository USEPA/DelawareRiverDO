...
source(here("raw","create_yoy_survey_data.R"))
...
# Combine data from Chester and Penn's Landing, calculate the percent
# survivorship on 10/31 by dividing the abundance by the initial abundance which
# was 10,000.
df <- rbind(
  sim.Chester.Zmin %>% 
  filter(month(date)==10 & day(date)==31) %>% # last day of simulation, October 31.
  mutate(survivorship=abundance*100/10000,
         Site="Chester") %>% 
...
pdf(file=here("figures","7.02 CPUE vs. Simulated Survivorship.pdf"),
