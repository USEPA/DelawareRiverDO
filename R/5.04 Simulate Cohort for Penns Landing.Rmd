...
transformArray <- function(inArray,vname) {
  doy <- seq(182,335)
  rownames(inArray) <- paste0("Y",c(seq(2002,2009),seq(2011,2022)))
  sim <- t(inArray) %>% 
    as.data.frame() %>% 
    cbind(doy,.) %>% 
...
#     file=paste0(dataPath,"5.04 Penns Landing Cohort Simulation 2002-2022.Rdata"))
...
plt <- ggplot()+
  geom_line(data=df,aes(x=as.Date(doy,origin=as.Date("2018-12-31")),y=weight,group=factor(year),linetype=year.with.capture))+
...
pdf(file=paste0(figurePath,"5.04 Predicted fish weight at PennsLanding.pdf"),
